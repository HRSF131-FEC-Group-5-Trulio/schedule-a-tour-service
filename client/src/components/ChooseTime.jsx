	...
	makeTimesOfDay() {
		...
		while (futureHour < 19) {
			const futureDate = new Date(
				date.getTime() + (currHour + 3) * 60 * 60000 + 30 * 60000 * i
			);
		...
		if (schedule) {
			schedule.forEach((app) => {
				if (app.date === selectedDate) {
					appointment.push(app.time);
				}
			});
		}

		const renderTime = [];
		timesOfDay.forEach((slot) => {
			if (appointment.indexOf(slot) < 0) {
				renderTime.push(slot);
			}
    });
